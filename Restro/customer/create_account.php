...
require_once('config/code-generator.php'); // Generates customer_id
...
if (isset($_POST['addCustomer'])) {
    ...
    if (empty($_POST["customer_phoneno"]) || empty($_POST["customer_name"]) || empty($_POST['customer_email']) || empty($_POST['customer_password']) || empty($_POST['customer_address'])) {
        ...
        $customer_password = sha1(md5($_POST['customer_password'])); // Hash the password for security
        $customer_address = $_POST['customer_address'];
        $customer_id = $_POST['customer_id']; // From the hidden input field
        ...
        if ($postStmt = $mysqli->prepare($postQuery)) {
            ...
            if ($postStmt->execute()) {
                ...
                header("refresh:1; url=index.php"); // Redirect to login after 1 second
                exit(); // Ensure no further code execution
            } else {
                $err = "Error executing query: " . $mysqli->error;
            }
        } else {
            $err = "Error preparing query: " . $mysqli->error;
        }
    }
}
                            ...
                            <form method="post" role="form">
                                ...
                                <div class="form-group mb-3">
                                    <div class="input-group input-group-alternative">
                                        ...
                                        <input class="form-control" required name="customer_address" placeholder="Address" type="text">
                                    </div>
                                </div>
